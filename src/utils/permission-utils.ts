...
export class PermissionUtils {
    ...
    public static isRoleBelowBotsHighestRole(role: Role, botMember: GuildMember): boolean {
        return botMember.roles.highest.comparePositionTo(role) > 0;
    }

    // TODO: Rename this to something to include giving and taking roles since the logic is the same
    public static async isRoleGivable(guild: Guild, role?: Role): Promise<boolean> {
        if (!role) return;
        ...
        return (
            ...
            this.isRoleBelowBotsHighestRole(role, botMember)
        );
    }
}
