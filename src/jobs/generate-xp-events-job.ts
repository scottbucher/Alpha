...
export class GenerateXpEventsJob extends Job {
    ...
    // Adjusted: (15% * 52 weekends) / 51 non-anniversary weekends ≈ 15.29%
    ...
    public async run(): Promise<void> {
        ...
        try {
            ...
            for (const guild of guilds) {
                try {
                    Logger.info(
                        Logs.info.generateXpEventsForGuild?.replaceAll(
                            '{GUILD_ID}',
                            guild.discordId
                        ) ||
                            `Generating XP events for guild ${guild.discordId} with time zone ${guild.generalSettings.timeZone || 'UTC'}`
                    );
    ...
    private async generateEventsForGuild(guild: GuildData, em: any): Promise<void> {
        ...
        Logger.info(
            `Current time for guild ${guild.discordId}: ${now.toISO()} (${now.toLocaleString(DateTime.DATETIME_FULL)})`
        );

        // Get the next 4 weekends
        const weekends = this.getNextWeekends(now, 4);
        Logger.info(
            `Generated ${weekends.length} weekend periods: ${weekends.map(w => `${w.start.toLocaleString()} - ${w.end.toLocaleString()}`).join(', ')}`
        );
        ...
        Logger.info(`Found ${existingEvents.length} existing event periods`);

        const newWeekends = weekends.filter(
            weekend => !this.hasExistingEvent(existingEvents, weekend.start, weekend.end)
        );

        Logger.info(`Found ${newWeekends.length} new weekend periods to consider for events`);
        ...
        for (const weekend of newWeekends) {
            ...
            if (isAnniversaryWeekend) {
                ...
                Logger.info(
                    `🎉 Anniversary XP Event generated for guild ${guild.discordId}: ${anniversaryInfo.years} year${anniversaryInfo.years !== 1 ? 's' : ''} (${multiplier}x) from ${weekend.start.toISO()} to ${weekend.end.toISO()}`
                );
